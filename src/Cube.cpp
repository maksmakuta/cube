...
namespace cube {
    ...
    void Cube::onRender(Renderer& r){
        if (m_show_debug) {
            const auto h = static_cast<float>(m_font.getSize()) * 1.15f;
            const auto pos = m_player.getPosition();
            const auto rot = m_player.getRotation();
            const auto chk = toChunk(pos);

            r.text(m_font,0xFFFFFFFF);
            r.print({0,h},"Cube v. 0.5.0");
            r.print({0,h*2},std::format("Position: [{:.2f},{:.2f},{:.2f}]",pos.x,pos.y,pos.z));
            r.print({0,h*3},std::format("Rotation: [{:.2f},{:.2f}]",rot.x,rot.y));
            r.print({0,h*5},std::format("Chunk: [{},{}]",chk.x,chk.y));
        }
    }

    void Cube::onRender(VoxelRenderer& r){
