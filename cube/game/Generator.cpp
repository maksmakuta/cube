...
namespace cube {
    ...
    constexpr auto BASE_HEIGHT = 72.f;
    constexpr auto WATER_LEVEL = 64.f;
    ...
    ChunkPtr Generator::generateChunk(const glm::ivec2& pos) const {
        std::vector heightmap(CHUNK_SIZE.x * CHUNK_SIZE.z,BASE_HEIGHT);
        const glm::vec2 world_pos = pos * glm::ivec2(CHUNK_SIZE.x,CHUNK_SIZE.z);
        ...
        for (auto x = 0; x < CHUNK_SIZE.x; x++) {
            for (auto z = 0; z < CHUNK_SIZE.z; z++) {
                ...
                int height = static_cast<int>(q * (CHUNK_SIZE.y - BASE_HEIGHT));
